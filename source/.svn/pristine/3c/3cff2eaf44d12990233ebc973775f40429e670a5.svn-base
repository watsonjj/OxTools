...
public:
	 ...
	 virtual Run_ptr Join(set<fs::path> inputs) {THROW("Join is not defined for file type");}
...
public:
	 virtual Run_ptr Read();
	 virtual Run_ptr Join(set<fs::path> inputs);
...
public:
	 ...
	 static Filepath_ptr Create(fs::path input_filepath);
};
