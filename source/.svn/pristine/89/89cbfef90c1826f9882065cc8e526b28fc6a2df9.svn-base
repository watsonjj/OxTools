...
void Run::AddPixel(uint32_t i, Pixel &ph) {
	...
	if ((itp.first->second).AddPixelEntry(ph)) {
		mNumPixelEntries++;
...
void Run::AddEntry(uint32_t i, Entry &eh) {
	...
	if ((itp.first->second).AddEventEntry(eh)) {
...
TH1F* Run::GetSpectrum(string branch) {
	if (!mSpectrum) {
		const char* name = Form("spectrum%d",mRunId);
		mSpectrum = new TH1F(name, name, 200, GetBranchMin(branch), GetBranchMax(branch));
...
void Run::UpdateCharges(string branch, string type, TH1F *calib_spectrum) {
	string run_identifier = pFile->GetRunIdentifier() + "_";
	fs::path output = pFile->GetOutputDirectory() / run_identifier;
	PulseSpectrumFit fit(GetSpectrum(branch), type, output, calib_spectrum);

	Progress progress(GetNumTotalEvents(), "Assigning True Charges : ",1);
	uint32_t count = 0;
	for (auto& camera_pair : GetCameraMap()) {
		auto& camera = camera_pair.second;
		for (auto& event_pair : camera.GetEventMap()) {
			...
			for (auto& entry_pair : event.GetEntryMap()) {
				auto& entry = entry_pair.second;
				float trueQ = fit.GetTrueCharge(entry.GetBranchValue(branch));
				float measuredQ = fit.ConvertMeasuredCharge(entry.GetBranchValue(branch));
				entry.mBranchValueMap["fTrueNPE"] = trueQ;
				entry.mBranchValueMap[branch] = measuredQ;
			}
		}
	}
}